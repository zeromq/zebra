...
################################################################################
*/
#ifndef Q_ZEB_CLIENT_H
#define Q_ZEB_CLIENT_H

#include "qzebra.h"

class QT_ZEBRA_EXPORT QZebClient : public QObject
{
    Q_OBJECT
public:

    //  Copy-construct to return the proper wrapped c types
    QZebClient (zeb_client_t *self, QObject *qObjParent = 0);

    //  Create a new zeb_client, return the reference if successful,   
    //  or NULL if construction failed due to lack of available memory.
    explicit QZebClient (QObject *qObjParent = 0);
    ...
    //  No explanation                                                                  
    //  Returns >= 0 if successful, -1 if interrupted.                                  
    int request (quint32 timeout, QZmsg *content);

    //  Send XRAP DELIVER message to server, takes ownership of message
    //  and destroys message when done sending it.                     
    int deliver (QZuuid *sender, QZmsg *content);
