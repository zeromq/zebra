...
//  register to a route; use the ENDPOINT command to configure the inferface
...
//      zsock_wait (dispatcher);
//
//  Register a handler to a route. A route can contain regular expression.
//  Note if a route contains parameters those are prepended as a packed
//  zhashx frame to the message, see ztrie_insert_route ().
//
