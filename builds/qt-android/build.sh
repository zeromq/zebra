...
##
# Make sure curl is built and copy the prefix

(android_build_verify_so "libcurl.so" &> /dev/null) || {
    # Use a default value assuming the curl project sits alongside this one
    test -z "$CURL_ROOT" && CURL_ROOT="$(cd ../../../curl && pwd)"
    
    if [ ! -d "$CURL_ROOT" ]; then
        echo "The CURL_ROOT directory does not exist"
        echo "  ${CURL_ROOT}"
        exit 1
    fi
    
    (bash ${CURL_ROOT}/builds/qt-android/build.sh) || exit 1
    UPSTREAM_PREFIX=${CURL_ROOT}/builds/qt-android/prefix/${TOOLCHAIN_NAME}
    cp -r ${UPSTREAM_PREFIX}/* ${ANDROID_BUILD_PREFIX}
}

##
# Build zwebrap from local source

(android_build_verify_so "libzwebrap.so" "libczmq.so" "libmicrohttpd.so" "libcurl.so" &> /dev/null) || {
    rm -rf "${cache}/zwebrap"
    (cp -r ../.. "${cache}/zwebrap" && cd "${cache}/zwebrap" \
        && make clean && rm configure config.status)
    rm 
    
    export LIBTOOL_EXTRA_LDFLAGS='-avoid-version'
    
    (cd "${cache}/zwebrap" && ./autogen.sh \
...
android_build_verify_so "libzwebrap.so" "libczmq.so" "libmicrohttpd.so" "libcurl.so"
